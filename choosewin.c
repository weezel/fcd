	...
	while ((c = getch()) != 'q') {
		...
		case 10: /* Enter */
			...
			if (offset >= MAX_CHOICESIZE)
				goto toolong;
			if (strlcpy(buf + offset, item_description(current_item(dirmenu)), MAX_CHOICESIZE - offset))
				goto toolong;

			pos_menu_cursor(dirmenu);
			/* XXX Some attention needed around these lines
			mvprintw(20, 0, "Item selected is : %s",
			    item_name(current_item(paths)));
			pos_menu_cursor(paths);
			*/
			break;
		}
	}
